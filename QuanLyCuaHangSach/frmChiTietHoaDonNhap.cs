...
namespace QuanLyCuaHangSach
{
    public partial class frmChiTietHoaDonNhap : Form
    {
        ...
        public frmChiTietHoaDonNhap(string maHD, bool isNhap)
        {
            InitializeComponent();
            _maHD = maHD;
            _isNhap = isNhap;
        ...
        private void LoadHeader()
        {
            ...
            // Lấy thêm các thông tin p.* (SĐT, Địa chỉ) để hiển thị lên form
            string sql = $@"SELECT h.*, nv.ho_ten, p.*, p.{(_isNhap ? "ten_nha_cung_cap" : "ten_khach_hang")} as TenDoiTac 
                            FROM {table} h 
            ...
            if (dt.Rows.Count > 0)
            {
                DataRow r = dt.Rows[0];
                txtMaHD.Text = _maHD;
                // Đồng bộ tên control với Designer: dtpNgay, txtNCC, txtTongTienHeader,...
                dtpNgay.Value = Convert.ToDateTime(r[_isNhap ? "ngay_nhap" : "ngay_dat"]);
                txtNhanVien.Text = r["ho_ten"].ToString();
                txtNCC.Text = r["TenDoiTac"].ToString();
                txtMaNCC.Text = r["ma_nha_cung_cap"].ToString();
